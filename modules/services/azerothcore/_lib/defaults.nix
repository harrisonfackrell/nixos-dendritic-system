# All default key/value pairs of AzerothCore's three core .conf files
...
#
# Every conf key - including dotted ones such as "Updates.EnableDatabases" -
# is a *flat* quoted attr name in the original dist-file order. The renderer
# in conf.nix writes them out verbatim, which is equivalent to the dist
# files: AzerothCore's Config::ParseFile strips all double quotes from values
# before using them.
#
# Values that depend on this host's configuration (ports, the *DatabaseInfo
# connection strings, the log/temp dirs and the mysql binary) are layered
# over the literals with `//` (right wins) at the end of each attrset.
#
# SourceDirectory is deliberately absent: it is the one key whose value is
# the package's own store path, supplied as the AC_SOURCE_DIRECTORY
# environment variable by the systemd units (see default.nix).
{ cfg, dbInfo, logsDir, tmpDir, mysqlExe }:

{ self, inputs, ... }: {
    ...
    services.llama-swap = {
        ...
        models = {
            ...
            "Qwen3.6 Sparse" = {
            cmd = "${llama-server} --no-ui -np 1 -kvu --port $\{PORT\} --fit-target 0 --jinja -m /etc/nixos/ai/models/text/qwopus3.6-35b-a3b-MTP-q5_K_M.gguf --spec-type draft-mtp --spec-draft-n-max 2";
            filters = {
                setParams = {
                temperature = 0.7;
                top_p = 0.80;

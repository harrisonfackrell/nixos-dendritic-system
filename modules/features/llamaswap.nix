      ...
      services.llama-swap = {
        ...
        in {
          ...
          models = {
            ...
            "GlisteningGem Dense" = {
              cmd = "${llama-server} --no-ui -np 1 -kvu --port $\{PORT\} --fit-target 0 --cache-ram 512 --ctx-checkpoints 16 --swa-full --jinja -m ${glistening-gem-dense} --mmproj ${gemma4-dense-mmproj}";
              filters = {
                setParams = {
                  temperature = 0.8;
                  min_p = 0.1;

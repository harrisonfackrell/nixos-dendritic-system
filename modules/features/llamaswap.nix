      ...
      services.llama-swap = {
        ...
        settings = let
          ...
          qwen38-dense-mmproj = pkgs.fetchurl {
            ...
            sha256 = "UuaBjk0Y7qAQxQ5SReqhCozD3MMO/qT/YMutir9WaeE=";
          };
        ...
        in {
          ...
          models = {
            ...
            # Programming
            "Qwen3.8 Dense" = {
              cmd = "${llama-server} --no-ui -np 1 -ctxcp 16 -cram 4096 -ctv q8_0 --fit-target 0 --port $\{PORT\} --jinja -m ${qwen38-dense} --mmproj ${qwen38-dense-mmproj} --spec-type draft-mtp --reasoning-preserve";
              filters = {
                setParams = {
                  temperature = 1.0;
                  top_p = 0.95;
            ...
            "Qwen3.6 Sparse" = {
              cmd = "${llama-server} --no-ui -np 1 -ctxcp 16 -cram 4096 --fit-target 0 --port $\{PORT\} --jinja -m ${qwen36-sparse} --mmproj ${qwen36-sparse-mmproj} --spec-type draft-mtp";
              filters = {
                setParams = {
                  temperature = 0.7;
                  top_p = 0.80;
